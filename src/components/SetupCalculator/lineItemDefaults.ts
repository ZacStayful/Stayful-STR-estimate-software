...
export interface LineItemDefinition {
  ...
  supplier: string;
  /** Quantity derivation: flat number, or function of bedroom count B */
...
// Canonical item definitions — a single source of truth.
export const DEFAULT_ITEMS: LineItemDefinition[] = [
  ...
  // Soft Furnishings & Décor
  { id: "soft_furnishings",  name: "Soft furnishings (per room)", category: "Soft Furnishings & Décor", supplier: "Dunelm",       qtyByBedrooms: (b) => b,     unitCost: 300 },
  { id: "clothes_rack",      name: "Clothes rack",            category: "Soft Furnishings & Décor",     supplier: "Amazon",       qtyByBedrooms: (b) => b,     unitCost: 20 },
  { id: "paint_per_room",    name: "Paint (per room)",        category: "Soft Furnishings & Décor",     supplier: "B&Q",          qtyByBedrooms: (b) => b,     unitCost: 100 },
  ...
  { id: "labour_zac",        name: "Labour — Zac (per day)",  category: "Services",                     supplier: "Internal",     qtyByBedrooms: ()  => 1,     unitCost: 350 },
  { id: "labour_mum",        name: "Labour — Mum (per day)",  category: "Services",                     supplier: "Internal",     qtyByBedrooms: ()  => 1,     unitCost: 200 },

  // Other
  { id: "keysafes",          name: "Keysafes",                category: "Other",                        supplier: "Amazon",       qtyByBedrooms: ()  => 2,     unitCost: 15 },
];

// Per-furnishing-state overrides. Keys are line item IDs. Each key can
// override `active` (toggle default on/off) and/or `qty` (replace the
// default qtyByBedrooms calc).
const FURNISHING_OVERRIDES: Record<FurnishingState, Record<string, { active?: boolean; qtyByBedrooms?: (b: number) => number }>> = {
  fully: {
    // Everything active with default qtys except zip-link singles
    zip_link_single: { active: false },
  },
  part: {
    // Fully-furnished baseline, but all "Furniture & Beds" items start inactive
  ...
  unfurnished: {
    // No furniture; tech/kitchen supplied separately by tenant; more labour
    ...
    coffee_table:    { active: false },
    dining_table:    { active: false },
    labour_zac:      { qtyByBedrooms: () => 1.2 },
    labour_mum:      { qtyByBedrooms: () => 1.2 },
  },
};
